...
class TopSetting extends MainPage {
  ...
  showInputGenerateObjects() {
    const button_input = document.getElementById('buttonGenerateObjects');
    const input_val = document.getElementById('inputGenerateObjects');
    if (
...
class BottomPanelTools extends MainPage {
  ...
  create_min_menu(new_div) {
    const new_div_menu_dropdown = document.createElement('DIV');
    new_div_menu_dropdown.setAttribute('id', 'dropdown');
    new_div.appendChild(new_div_menu_dropdown);
    const new_div_menu_dropdown_btn = document.createElement('BUTTON');
    ...
    const new_div_menu_start_dropdown = document.createElement('DIV');
    ...
    const new_div_menu_a_strong_connect = document.createElement('A');
    ...
    const new_div_menu_a_line_prog = document.createElement('A');
  ...
  create_set_nodes(count_node) {
    const was_count_nodes = this.get_was_count_node;
    const need_count_nodes = was_count_nodes + parseInt(count_node);
    const start_tag = document.getElementById('init_date');
    let new_div; let old_p; let new_p; let
      new_br;
    ...
    document.getElementById('show_now_count_nodes').innerHTML = this.get_was_count_node - this.get_now_count_node;
  }
}
...
class Field extends MainPage {
  ...
  get object_inside() {
    const elems = document.getElementsByClassName('node draggable');
    const include_elems = [];
    for (let i = 0; i < elems.length; i++) {
      const rect_elem = elems[i].getBoundingClientRect();
      if (
  ...
  change_state_field() {
    ...
    const nodes_include = this.object_inside;
...
class SetNodes {
  ...
  moveItemToXY(item, x, y, offX, offY) {
    ...
    const minX = 0;
    const minY = 0;
    const maxY = document.documentElement.clientHeight - item.offsetHeight;
    const maxX = document.documentElement.clientWidth - item.offsetWidth;
    let itemX = x - offX;
    let itemY = y - offY;
...
class Data {
  ...
  save_data(node, value) {
    ...
    const canvas_elem = document.getElementById('field');
    const x_node = parseFloat(node.offsetLeft)
      + parseFloat(node.offsetWidth) / 2
      - parseFloat(canvas_elem.offsetLeft);
    const y_node = parseFloat(node.offsetTop)
...
class PopUp extends Data {
  ...
  create_display_popup(node, value) {
    const display_popup = document.createElement('DIV');
    ...
    const popup_exit = document.createElement('DIV');
  ...
  create_popup(e) {
    ...
    const popup_input = document.createElement('INPUT');
    ...
    for (let i = 0; i < loop_popup; i++) {
      for (let j = 0; j < loop_display; j++) {
        if (
          ...
          const { value } = this.select_popup_input()[i];
          const node = this.select_popup_body()[i].parentElement;
          ...
          this.select_popup_body()[i].style.backgroundColor = 'rgba(255, 150, 150, 0.5)';
          const random_int = D.getRandomInt(0, 100);
          ...
          this.select_popup_body()[i].style.backgroundColor = 'rgba(150, 150, 255, 0.5)';
          break;
        } else if (
          ...
          const node = this.select_display_body()[j].parentElement;
...
class SetWeight {
  ...
  create_weight() {
    const body_canvas = document.getElementById('field');
    const popup_weight = document.createElement('DIV');
    popup_weight.setAttribute('class', 'popup_weight');

    let existing = CLN.get_existing_localStorage(this.node_1);
    const start_pos = CLN.get_pos_node(existing);
    existing = CLN.get_existing_localStorage(this.node_2);
    const line_coord = CLN.get_pos_node(existing);

    popup_weight.style.top = `${((parseInt(start_pos.y) + parseInt(line_coord.y)) / 2 + 155).toString()
    }px`;
    popup_weight.style.left = `${((parseInt(start_pos.x) + parseInt(line_coord.x)) / 2 + 60).toString()
    }px`;
    body_canvas.after(popup_weight);

    const weight_input_1 = document.createElement('INPUT');
    ...
    const weight_input_2 = document.createElement('INPUT');
  ...
  create_display_weight(value_1, value_2) {
    const body_canvas = document.getElementById('field');
    const display_weight = document.createElement('DIV');
    ...
    display_weight.style.top = `${((parseInt(start_pos.y) + parseInt(line_coord.y)) / 2 + 155).toString()
    }px`;
    display_weight.style.left = `${((parseInt(start_pos.x) + parseInt(line_coord.x)) / 2).toString()
    }px`;
    body_canvas.after(display_weight);
  }

  save_weight(value_1, value_2) {
    const existing_node_1 = CLN.get_existing_localStorage(this.node_1);
    existing_node_1.push(value_1);
    localStorage.setItem(this.node_1, existing_node_1.toString());
    const existing_node_2 = CLN.get_existing_localStorage(this.node_2);
    ...
    for (let i = 0; i < loop_weight; i++) {
      if (e.target.className == 'weight_button') {
        if (
          ...
          const value_1 = this.select_weight_input_1()[i].value;
          const value_2 = this.select_weight_input_2()[i].value;
...
class DrawLine {
  ...
  get_client_offset(e) {
    ...
    const x = pageX - this.canvas_elem.offsetLeft;
    const y = pageY - this.canvas_elem.offsetTop;
    return { x, y };
  }
  ...
  start_draw() {
    this.canvas_elem.addEventListener('mousedown', (e) => {
      DL.mouse_down_listener(e);
      CLN.get_coord_line(e);
    });
    this.canvas_elem.addEventListener('mousemove', (e) => {
      DL.mouse_move_listener(e);
    });
    this.canvas_elem.addEventListener('mouseup', (e) => {
      DL.mouse_up_listener(e);
      CLN.get_coord_line(e);
    });
    this.canvas_elem.addEventListener('touchstart', (e) => {
      DL.mouse_down_listener(e);
    });
    this.canvas_elem.addEventListener('touchmove', (e) => {
      DL.mouse_move_listener(e);
    });
    this.canvas_elem.addEventListener('touchend', (e) => {
      DL.mouse_up_listener(e);
    });
  }
}
...
class ConcatLineNodes extends DrawLine {
  ...
  Add_binding_localstorage_Draw_line(pair_nodes_0, pair_nodes_1) {
    ...
    const start_pos = this.get_pos_node(existing);
    ...
    const line_coord = this.get_pos_node(existing);
  ...
  binding(nearest_node) {
    ...
    if (
      ...
      for (let i = 0; i < this.pair_nodes.length - 2; i += 2) {
        if (
  ...
  affiliation(point_line) {
    ...
    const x_line = parseFloat(point_line.x);
    const y_line = parseFloat(point_line.y);
  ...
  get_coord_line(e) {
    ...
    if (this.nodes.length != 0) {
      const point_line = this.get_client_offset(e);
      const nearest_node = this.affiliation(point_line);
